...
def word_count(sentence):
    ...
    while char < len(sentence):
        if sentence[char] not in punctuations:
            filtered_sentence += sentence[char].lower()
        char += 1
...
def missing_number(n, arr):
    ...
    for char in arr:
        count[char] = count.get(char, 0) + 1
    result = [num for num in range(1, n + 1) if num not in count]
    return result
...
def letter_sort(string):
    ...
    for i in range(len(chars)):
        sorted_string += chr(i + 97) * (chars[i])
