...
class Matrix:
    ...
    def printer(self):
        for row in range(self.m):
            print(self.storage[row])
        print

    def isValid(self, i, j):
        return not (i < 0 or j < 0 or i >= self.m or j >= self.n)
    ...
    def scale(self, factor):
        for row in range(self.m):
          for col in range(self.n):
            newVal = self.retrieve(row, col) * factor
            self.insert(row, col, newVal)
    ...
    def flatten(self):
        ...
        for row in range(self.m):
          for col in range(self.n):
            result.append(self.storage[row][col])
        return result

    def slice(self, rowRange, colRange):
        rowMin = max(0, rowRange[0])
        rowMax = min(rowRange[1], self.m)
        colMin = max(0, colRange[0])
        colMax = min(colRange[1], self.n)

        newMatrix = Matrix(rowMax - rowMin, colMax - colMin)
        for i in range(rowMin, rowMax):
          for j in range(colMin, colMax):
            newVal = self.retrieve(i, j)
            newMatrix.insert(i - rowMin, j - colMin, newVal)
        return newMatrix

    def transpose(self):
        newMatrix = Matrix(self.n, self.m)
        for row in range(self.m):
          for col in range(self.n):
            val = self.retrieve(row, col)
            newMatrix.insert(col, row, val)
        return newMatrix
    ...
    def multiply(self, matrix):
        ...
        newMatrix = Matrix(self.m, matrix.n)
        for i in range(newMatrix.m):
            for j  in range(newMatrix.n):
              newVal = 0
              for k in range(self.n):
                newVal += self.retrieve(i, k) * matrix.retrieve(k, j)
